...
| `ai4s-skills` | `ai4s-research/ai4s-skills` | MIT (confirmed via GitHub API) | **No** — fetched into `runtime/skills/external/`, which is not in `tauri.conf.json`'s `bundle.resources`; not embedded in any built installer today |
| `anthropic-skills` (docx/pdf/pptx/xlsx) | `anthropics/skills` | **Proprietary** — each skill's own `LICENSE.txt` reads "(c) Anthropic, PBC. All rights reserved... governed by your agreement with Anthropic regarding use of Anthropic's services" (verified directly this session — a prior in-repo comment incorrectly called this Apache-2.0, corrected in `scripts/dev/fetch-skills.sh` this session) | **No** — same reason as `ai4s-skills`: not in `bundle.resources`, not embedded today |
