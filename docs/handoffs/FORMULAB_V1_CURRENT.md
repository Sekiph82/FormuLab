...
- `python -m pytest runtime/pipeline -q` — 320/320 (pre full-text-gate-
  policy correction; re-run and record the corrected count under
  FVL-01.005 before starting FVL-02).
- `cargo test --release` (full workspace) — 342/342.
- `pnpm vitest run` — 138 files / 1248 tests.
- `git diff --check` — clean.

## Latest commit SHA

`8bfc11b04142fa30c623c37ca8d7b01d58d0797b` (pushed to
`origin/feature/laboratory-stability`). The same-day full-text-gate policy
correction and this tracker's own creation land in a separate, later
commit — see `git log` for the exact current HEAD.
