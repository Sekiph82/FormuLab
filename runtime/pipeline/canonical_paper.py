...
SOURCE_AVAILABILITY: Dict[str, Dict[str, str]] = {
    ...
    "crossref": {"status": "existing", "note": "discover.fetch_crossref, already live and default-on — architecture doc §3 lists it under Findpapers, but a working native fetcher already exists; Session 1 decides keep-vs-replace"},
    ...
    "pubmed": {"status": "planned_findpapers", "note": "not yet built; Europe PMC's europepmc source already covers PubMed/MED records today, so confirm real incremental coverage before building a separate adapter"},
    "semantic_scholar": {"status": "planned_findpapers", "note": "not yet built"},
    "core": {"status": "planned_native", "note": "not yet built"},
    "doaj": {"status": "planned_native", "note": "not yet built"},
    "base": {"status": "planned_native", "note": "not yet built"},
    "unpaywall": {"status": "planned_native", "note": "not yet built — also the OA full-text resolution mechanism architecture doc §4 names, not just a discovery source"},
