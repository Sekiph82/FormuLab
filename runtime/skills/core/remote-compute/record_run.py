#!/usr/bin/env python3
"""Record a remote (SSH/HPC/Modal) experiment run into the FormuLab provenance.
...
record to <workspace>/.FormuLab/remote-runs.jsonl, which the app merges into
...
STORE = os.path.join(".FormuLab", "remote-runs.jsonl")
ENV_DIR = os.path.join(".FormuLab", "env")  # package lockfiles, content-addressed
...
def read_env(path):
    ...
    .FormuLab/env/<hash>.txt (mirroring how the app records local runs), and
    ...
    env: dict[str, object] = {
        ...
        "app": os.environ.get("FormuLab_APP_VERSION", "unknown"),
    }
    if python:
        env["python"] = python
    if freeze:
...
def main():
    p = argparse.ArgumentParser(description="Record a remote run into FormuLab provenance.")
    ...
    # The skill passes `--session-id "$(cat .FormuLab/session.txt)"`, which is
