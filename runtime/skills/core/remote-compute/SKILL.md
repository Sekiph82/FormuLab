...
1. `cat .FormuLab/compute.json` in the workspace (the app keeps this file in
...
1. Pick a job name and build the remote dir path (remember the literal string —
   ...
   `REMOTE=FormuLab/jobs/<name>-<YYYYmmdd-HHMMSS>`
...
2. Submit:
   ```bash
   REMOTE=FormuLab/jobs/<job-name>-$(date +%Y%m%d-%H%M%S)
...
python "$XDG_CONFIG_HOME/opencode/skills/remote-compute/record_run.py" \
  ...
  --session-id "$(cat .FormuLab/session.txt 2>/dev/null)"
```
