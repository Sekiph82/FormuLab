...
fn db_path(root: &Path) -> PathBuf {
    root.join(".FormuLab").join(DB_FILE)
}
...
/// Every runs log under the base folder: the base's own `.FormuLab/` plus
...
fn source_files(base: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(entries) = std::fs::read_dir(base) {
        for e in entries.flatten() {
            ...
            // .FormuLab store lives there) and files.
    ...
    dirs.into_iter()
        .flat_map(|d| [RUNS_FILE, REMOTE_RUNS_FILE].map(|f| d.join(".FormuLab").join(f)))
        .collect()
}
...
mod tests {
    ...
    fn write_log(root: &Path, file: &str, lines: &[&str]) {
        ...
        let mut f = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(root.join(".FormuLab").join(file))
            .unwrap();
        for l in lines {
            writeln!(f, "{l}").unwrap();
        }
    ...
    fn merges_remote_runs_and_a_partial_last_line_is_deferred() {
        ...
        use std::io::Write;
        let mut f = std::fs::OpenOptions::new().append(true).open(root.join(".FormuLab").join(RUNS_FILE)).unwrap();
    ...
    fn indexes_runs_across_session_subfolders_and_filters_by_session() {
        ...
        for (sess, ts) in [("ses_a", 100), ("ses_b", 200)] {
            let dir = root.join(sess).join(".FormuLab");
