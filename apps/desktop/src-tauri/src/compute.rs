...
// <workspace>/.FormuLab/compute.json (see compute_machines below).
...
fn compute_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    // agent reaches it from its session folder via `../.FormuLab/compute.json`
    // (the seeded harness points at it), so no per-session copies are kept.
    Ok(base_workspace_dir(app)?.join(".FormuLab").join(COMPUTE_FILE))
}
...
fn load_machines(app: &AppHandle) -> Result<Vec<Machine>, String> {
    ...
    let legacy = base_workspace_dir(app)?.join(".FormuLab").join("hpc.json");
...
/// workspace's `.FormuLab/compute.json` so the agent reads it IN-workspace —
...
pub(crate) fn materialize_active(app: &AppHandle) {
    ...
    write_materialized(
        &base.join(".FormuLab").join(COMPUTE_FILE),
        &active.join(".FormuLab"),
    );
}
...
";
    ...
    fn materialize_copies_then_clears() {
        ...
        let base_os = root.join("base-FormuLab");
        let sess_os = root.join("session-FormuLab");
