...
// version record to <workspace>/.FormuLab/provenance.jsonl — append-only,
...
pub struct EnvInfo {
    ...
    pub platform: String,
    /// FormuLab app version that recorded this.
    pub app: String,
...
/// `.FormuLab/env/<hash>.txt`; records carry only the count + hash so the
...
/// Read a content-addressed package lockfile (`.FormuLab/env/<hash>.txt`).
...
mod tests {
    ...
    fn lockfile_is_content_addressed_and_deduped() {
        ...
        let lock = root.join(".FormuLab/env").join(format!("{}.txt", s1.hash));
    ...
    fn corrupt_lines_are_skipped_and_content_is_capped() {
        ...
        use std::io::Write;
        let file = root.join(".FormuLab/provenance.jsonl");
