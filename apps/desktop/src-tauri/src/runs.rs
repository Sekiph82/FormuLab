...
// <workspace>/.FormuLab/runs.jsonl: the command, code version (entry scripts
...
/// Read a content-addressed run log (`.FormuLab/logs/<hash>.txt`). `hash` is
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("formulab-runs-{tag}-{}", std::process::id()));
    ...
    fn detects_only_files_modified_in_the_window() {
        ...
        std::fs::write(root.join(".FormuLab/ignore.txt"), "x").unwrap();
        ...
        assert!(!paths.iter().any(|p| p.starts_with(".FormuLab")));
