 ...
 * whose authoritative Compatibility or Safety Engine verdict is
 * `"blocked"` can never be crowned cheapest merely because its price is
 * attractive (task §9/§12) — a warning or unknown state does NOT exclude
 * a version, matching the real platform's own "only `blocking` is a hard
 * block" semantics (confirmed by `blockingExclusionConstraints`/
 * `SubstitutionPanel.tsx`'s `hasBlockingCompatibilityFinding`/
 * `hasBlockingSafetyFinding`).
 */
