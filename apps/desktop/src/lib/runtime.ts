...
async function performTurn(
  ...
  try {
    ...
    if (!id) {
      ...
      // own fresh dated folder (~/Documents/FormuLab/<date-time>) first,
 ...
 * relative path (`demo/analyze.py`), not the full `/Users/.../FormuLab/...`
 ...
 * ending in `FormuLab/` matches it whether or not it has a leading slash
 * (OpenCode's write-tool titles drop it).
 */
export function tidyToolTitle(title: string): string {
  return title.replace(/[^\s]*FormuLab\//g, "").trim() || title;
}
