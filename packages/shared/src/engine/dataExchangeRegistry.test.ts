...
describe("Data Exchange template registry", () => {
  it("registers exactly the 24 mandated templates plus the 11 Reverse Formulation templates plus the 6 Phase 8 dossier-expansion templates plus the 4 FVL-04.005/.007/.008/.011 operational templates (finished_product_specifications, inventory_records, exchange_rates, material_suppliers)", () => {
    expect(DATA_EXCHANGE_TEMPLATES).toHaveLength(45);
  });
  ...
  it("lists all templates", () => {
    expect(listDataExchangeTemplates().length).toBe(45);
  });
